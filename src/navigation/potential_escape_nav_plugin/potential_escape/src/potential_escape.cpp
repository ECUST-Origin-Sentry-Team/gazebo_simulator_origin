...
#include "potential_escape/potential_escape.hpp"
...
namespace nav2_behaviors
{
  ...
  Status Escape::onRun(const std::shared_ptr<const EscapeAction::Goal> command)
  {
    ...
    command_speed = std::fabs(command->speed);
  ...
  Status Escape::onCycleUpdate()
  {
    ...
    cmd_vel->linear.x = command_speed / hypot(std::fabs(command_x_), std::fabs(command_y_)) * std::fabs(command_x_) * (command_x_ > 0 ? 1.0 : -1.0) * 5;
    cmd_vel->linear.y = command_speed / hypot(std::fabs(command_x_), std::fabs(command_y_)) * std::fabs(command_y_) * (command_y_ > 0 ? 1.0 : -1.0) * 5;
