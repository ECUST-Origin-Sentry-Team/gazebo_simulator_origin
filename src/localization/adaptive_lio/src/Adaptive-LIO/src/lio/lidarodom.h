...
namespace zjloc
{
     ...
     struct CTPointToPlaneResidual
     {
          Eigen::Vector3d raw_point;   // 原始点
          Eigen::Vector3d plane_point; // 对应平面点
          Eigen::Vector3d normal;      // 法向量
          double alpha_time;           // 时间权重
          double weight;               // 残差权重

          CTPointToPlaneResidual(const Eigen::Vector3d &raw,
                                 const Eigen::Vector3d &plane,
                                 const Eigen::Vector3d &n,
                                 double alpha, double w)
              : raw_point(raw), plane_point(plane), normal(n), alpha_time(alpha), weight(w) {}

          template <typename T>
          bool operator()(const T *begin_q_ptr, const T *end_q_ptr,
                          const T *begin_t_ptr, const T *end_t_ptr,
                          T *residual) const
          {
               Eigen::Map<const Eigen::Quaternion<T>> begin_q(begin_q_ptr);
               Eigen::Map<const Eigen::Quaternion<T>> end_q(end_q_ptr);
               Eigen::Map<const Eigen::Matrix<T, 3, 1>> begin_t(begin_t_ptr);
               Eigen::Map<const Eigen::Matrix<T, 3, 1>> end_t(end_t_ptr);

               // 插值
               Eigen::Quaternion<T> q_interp = begin_q.slerp(T(alpha_time), end_q);
               Eigen::Matrix<T, 3, 1> t_interp = (T(1) - T(alpha_time)) * begin_t + T(alpha_time) * end_t;

               Eigen::Matrix<T, 3, 1> p_transformed = q_interp * raw_point.cast<T>() + t_interp;

               residual[0] = T(weight) * normal.cast<T>().dot(p_transformed - plane_point.cast<T>());
               return true;
          }

          static ceres::CostFunction *Create(const Eigen::Vector3d &raw,
                                             const Eigen::Vector3d &plane,
                                             const Eigen::Vector3d &normal,
                                             double alpha,
                                             double weight)
          {
               return new ceres::AutoDiffCostFunction<CTPointToPlaneResidual, 1, 4, 4, 3, 3>(
                   new CTPointToPlaneResidual(raw, plane, normal, alpha, weight));
          }
     };
