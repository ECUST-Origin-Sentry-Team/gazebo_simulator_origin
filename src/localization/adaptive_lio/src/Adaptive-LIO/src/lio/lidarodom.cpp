...
namespace zjloc
{
     ...
     void lidarodom_m::addSurfCostFactor(std::vector<ceres::CostFunction *> &surf,
                                         ...
                                         const cloudFrame *p_frame)
     {
          ...
          for (auto &keypoint : keypoints)
          {
               ...
               // 估计法向量和平面性
               auto neighborhood = computeNeighborhoodDistribution(vector_neighbors);
               Eigen::Vector3d norm_vector = neighborhood.normal.normalized();
               normals.push_back(norm_vector);

               double weight = std::pow(neighborhood.a2D, options_.power_planarity);
               ...
               {
                    double point_to_plane_dist =
                        std::abs((keypoint.point - vector_neighbors[i]).transpose() * norm_vector);
                    if (point_to_plane_dist < options_.max_dist_to_plane_icp)
                    {
                         num_residuals++;
                         ...
                         if (options_.icpmodel == IcpModel::CT_POINT_TO_PLANE)
                         {
                              // 创建 CTPointToPlaneResidual
                              auto *cost_function = zjloc::CTPointToPlaneResidual::Create(
                                  keypoint.raw_point,  // 原始点
                                  vector_neighbors[i], // 最近邻平面点
                                  norm_vector,         // 法向量
                                  keypoint.alpha_time, // 插值时间
                                  weight               // 权重
                              );
                         ...
                         {
                              // 原始 POINT_TO_PLANE 写法，可能不考虑 IMU 插值
                              Eigen::Vector3d point_end =
                                  p_frame->p_state->rotation.inverse() * keypoint.point -
                                  p_frame->p_state->rotation.inverse() * p_frame->p_state->translation;

                              auto *cost_function = CT_ICP::PointToPlaneFunctor::Create(
                                  vector_neighbors[i], point_end, norm_vector, weight);
