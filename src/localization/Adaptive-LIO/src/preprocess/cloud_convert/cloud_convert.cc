...
namespace zjloc
{
    ...
    {
#if defined(LIVOX_ROS2_DRIVER) || defined(LIVOX_ROS1_DRIVER)
    AviaHandler(msg);
#else
    (void)msg; // no-op when no livox driver available
#endif
    pcl_out = cloud_out_;
    }
...
#else
    void CloudConvert::AviaHandler(const void * /*msg*/)
    {
        // No livox driver available: keep outputs empty.
        cloud_out_.clear();
        cloud_full_.clear();
        return;
    }
